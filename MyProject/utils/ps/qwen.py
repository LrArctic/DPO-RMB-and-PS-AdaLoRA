import json
import re
import matplotlib.pyplot as plt
import numpy as np
from pathlib import Path

# =========================
# 路径配置
# =========================
# 建议检查此路径下的 json，确认 Qwen 的层数索引最大值
CONFIG_PATH = "/home/data/students_user/2024/lr/code/MyProject/output/22/adapter/adapter_config.json"

SCRIPT_DIR = Path(__file__).resolve().parent
OUTPUT_PATH = SCRIPT_DIR / "qwen_sensitivity.pdf"

# =========================
# 读取 JSON
# =========================
with open(CONFIG_PATH, "r") as f:
    config = json.load(f)

sensitivity_cache = config.get("_dpo_sensitivity_cache", {})

# =========================
# 按 layer 收集 q / v (Qwen2.5-7B 标准为 28 层)
# =========================
num_layers = 28 
q_values = [0.0] * num_layers
v_values = [0.0] * num_layers

# 匹配正则：Qwen 的命名空间通常与 Llama 一致
pattern = re.compile(r"model\.layers\.(\d+)\.self_attn\.(q_proj|v_proj)")

for name, value in sensitivity_cache.items():
    match = pattern.match(name)
    if match:
        layer_idx = int(match.group(1))
        # 仅记录 28 层以内的有效层，防止索引越界
        if layer_idx < num_layers:
            proj_type = match.group(2)
            if proj_type == "q_proj":
                q_values[layer_idx] = value
            else:
                v_values[layer_idx] = value

# =========================
# 纵坐标换算 (例如放大 100 倍以便于观察百分比差异)
# =========================
SCALE_FACTOR = 100
q_values_scaled = np.array(q_values) * SCALE_FACTOR
v_values_scaled = np.array(v_values) * SCALE_FACTOR

layers = np.arange(num_layers)

# =========================
# 绘图 (SCI 风格)
# =========================
plt.figure(figsize=(9, 4.5))

# 使用实线和标记点增强可读性
plt.plot(
    layers, 
    q_values_scaled, 
    color='#1f77b4', 
    marker='o', 
    markersize=4,
    linewidth=1.5, 
    label=r"$W_q$ Sensitivity ($\times 10^{-2}$)"
)

plt.plot(
    layers, 
    v_values_scaled, 
    color='#ff7f0e', 
    linestyle="--", 
    marker='s', 
    markersize=4,
    linewidth=1.5, 
    label=r"$W_v$ Sensitivity ($\times 10^{-2}$)"
)

# 针对最后几层崩塌现象，添加视觉标注说明（可选）
if v_values_scaled[-1] < 1e-4:
    plt.annotate('Sensitivity Collapse', xy=(num_layers-1, 0), xytext=(num_layers-6, 0.5),
                 arrowprops=dict(facecolor='black', shrink=0.05, width=1, headwidth=5))

plt.xlabel("Layer Index", fontsize=12)
plt.ylabel("Scaled Sensitivity Value", fontsize=12)
plt.title("Preference Sensitivity Distribution of Qwen2.5-7B-Instruct", fontsize=13)

# 设置刻度
plt.xticks(np.arange(0, num_layers, 2))
plt.xlim(-0.5, num_layers - 0.5)

plt.legend(frameon=True, loc='lower right')
plt.grid(True, linestyle=':', alpha=0.6)

plt.tight_layout()

# =========================
# 保存图片
# =========================
plt.savefig(OUTPUT_PATH, bbox_inches="tight")
plt.show()

print(f"Figure saved to: {OUTPUT_PATH}")