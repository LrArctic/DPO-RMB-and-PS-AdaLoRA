...
from peft import AdaLoraConfig, get_peft_model,PeftModel# 新增
...
ada_lora_config = AdaLoraConfig(
    ...
    orth_reg_weight=0.1, # 正交正则化强度0.5
...
RMB_config = RMBConfig(
    enabled=True, # 启用 RMB
    size=8000, # buffer 容量（推荐 5k-50k）
    batch_size=32, # 每步从 buffer 抽多少条来 mix
    mix_ratio=0.05, # 历史 loss 占最终 loss 的比例（0.0 - 1.0）
...
training_args = DPOConfig(
    ...
    gradient_accumulation_steps=2, # 梯度累积2
    num_train_epochs=3,
    max_length=1024,
    ...
    logging_dir="./version/logs", # tensorboard 日志目录
